...
#if defined(_LITE_VERSION) && !defined(_STAND_ALONE)
    if (__mg_ptmi == NULL && mgIsServer) {
        ClientCanDrawNowEx (FALSE, NULL);
    }
#endif
...
#if defined(_LITE_VERSION) && !defined(_STAND_ALONE)
        RECT rcTemp;
        IntersectRect (&rcTemp, &g_rcDesktop, (const RECT*)&SHAREDRES_CLI_SCR_LX);
        if (IntersectRect (&rcTemp, &pcrc->rc, &rcTemp)) {
            if (!IntersectRect (&rcMove, &rcTemp, &rcScreen)) {
                pcrc = pcrc->next;
                continue;
            }
        }
...
#if defined(_LITE_VERSION) && !defined(_STAND_ALONE)
    IntersectRect (&rcInter, &g_rcDesktop, (const RECT*)&SHAREDRES_CLI_SCR_LX);
    nInvCount = SubtractRect (rcInv, &src, &rcInter);
#else
    ...
    switch (message) {
        case MSG_MOUSEMOVE:
            ...
            if (pUnderPointer == NULL) {
                ...
                if (pOldUnderPointer != pUnderPointer) {
                    if (pOldUnderPointer) {
                        PostMessage ((HWND)pOldUnderPointer,
                    ...
                    PostMessage ((HWND)pUnderPointer,
                        MSG_MOUSEMOVEIN, TRUE, (LPARAM)pOldUnderPointer);
        ...
        case MSG_GETNEXTMAINWIN:
        {
            ...
            if (pWin) {
                ...
                if (pNode)
                    ...
                    return sg_MainWinZOrder.pTopMost->hWnd;
            }
            else {
                if (sg_TopMostWinZOrder.pTopMost->hWnd)
                    return sg_TopMostWinZOrder.pTopMost->hWnd;
                else
                    return sg_MainWinZOrder.pTopMost->hWnd;
            }
            break;
...
int DesktopWinProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
       ...
       break;
        ...
        case MSG_DT_KEYDOWN ... MSG_DT_SYSKEYUP:
