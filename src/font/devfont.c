/*
** $Id: devfont.c,v 1.23 2003/11/23 04:09:08 weiym Exp $
** 
...
#if 0
            fprintf (stderr, "family: %s, requested: %s; ", family, log_font->family);
#endif
...
#if 0
        fprintf (stderr, "select a font: %s\n", matched_font->name);
#endif
#ifndef HAVE_ALLOCA
    FreeFixStr (match_bits);
#endif
...
DEVFONT* GetMatchedMBDevFont (LOGFONT* log_font)
{
    DEVFONT* dev_font;
    int i;
    //BYTE* match_bits = alloca (nr_mb_dev_fonts);
    BYTE* match_bits;
    char charset_req [LEN_FONT_NAME + 1];
    int min_error;
    DEVFONT* matched_font;
