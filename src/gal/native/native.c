/*
** $Id: native.c,v 1.56 2003/11/22 11:49:29 weiym Exp $
**
...
static int set_effective_clip_rect (PSD psd)
{
    if (__mg_saved_clip_minx < SHAREDRES_CLI_SCR_LX)
        psd->clipminx = SHAREDRES_CLI_SCR_LX;
    else
        psd->clipminx = __mg_saved_clip_minx;

    if (__mg_saved_clip_miny < SHAREDRES_CLI_SCR_TY)
        psd->clipminy = SHAREDRES_CLI_SCR_TY;
    else
        psd->clipminy = __mg_saved_clip_miny;
    
    if (__mg_saved_clip_maxx > (SHAREDRES_CLI_SCR_RX + 1))
        psd->clipmaxx = SHAREDRES_CLI_SCR_RX;
    else
        psd->clipmaxx = __mg_saved_clip_maxx;

    if (__mg_saved_clip_maxy > (SHAREDRES_CLI_SCR_BY + 1))
        psd->clipmaxy = SHAREDRES_CLI_SCR_BY;
    else
        psd->clipmaxy = __mg_saved_clip_maxy;

    if (psd->clipmaxx < psd->clipminx)
        return -1;
    else if (psd->clipmaxy < psd->clipminy)
        return -1;
...
#define BLOCK_DRAW_SEM                                  \
    ...
    if (((!mgIsServer && (SHAREDRES_TOPMOST_LAYER != __mg_layer)) || __mg_switch_away)) \
        goto leave_drawing;                             \
    if (!mgIsServer && cur_gfx->phygc.psd == gc.psd)    \
        if (set_effective_clip_rect (gc.psd))                 \
            goto leave_drawing;
...
static int setclipping (GAL_GC gc, int x1, int y1, int x2, int y2)
{
    ...
    psd->doclip = 1;
#if defined(_LITE_VERSION) && !(_STAND_ALONE)
    if (!mgIsServer && psd == cur_gfx->phygc.psd) {
        ...
        GAL_GC dst, int dx, int dy)
{
    ...
    if (_COOR_TRANS && src.psd == cur_gfx->phygc.psd) {
        ...
        return getbox_wrapper (src, sx, sy, w, h, 
                        dst.psd->addr + dst.psd->linelen * dy + Bpp * dx, 
    ...
    if (_COOR_TRANS && dst.psd == cur_gfx->phygc.psd) {
        ...
        return putbox_wrapper (dst, dx, dy, w, h, 
                            src.psd->addr + src.psd->linelen * sy + Bpp * sx, 
...
BOOL InitNative (GFX* gfx)
{
    ...
    else if (strcasecmp (gfx->id, "fbcon") == 0)
        psd = scrdev.Open (&scrdev);
#endif
#ifdef _NATIVE_GAL_QVFB
    else if (strcasecmp (gfx->id, "qvfb") == 0)
        psd = qvfbdev.Open (&qvfbdev);
#endif
#ifdef _NATIVE_GAL_ECOSLCD
    else if (strcasecmp (gfx->id, "ecoslcd") == 0)
        psd = ecoslcd.Open (&ecoslcd);
#endif
