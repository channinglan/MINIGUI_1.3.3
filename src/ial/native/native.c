/*
** $Id: native.c,v 1.47 2003/09/04 03:38:26 weiym Exp $
**
...
BOOL InitNativeInput (INPUT* input, const char* mdev, const char* mtype)
{
    ...
    if (strncasecmp (mtype, "none", 4) == 0) {
        mousedev = NULL;
        goto found;
    }
#ifdef _IMPS2_SUPPORT
    if (strncasecmp (mtype, "imps2", 5) == 0 ) {
        mousedev = &mousedev_IMPS2;
        goto found;
    }
#endif
#ifdef _PS2_SUPPORT
    if (strncasecmp (mtype, "ps2", 3) == 0 ) {
        mousedev = &mousedev_PS2;
        goto found;
    }
#endif
#ifdef _GPM_SUPPORT
    if (strncasecmp (mtype, "gpm", 3) == 0) {
        mousedev = &mousedev_GPM;
        goto found;
    }
#endif
#ifdef _MS_SUPPORT
    if (strncasecmp (mtype, "ms", 3) == 0) {
        mousedev = &mousedev_MS;
        goto found;
    }
#endif
#ifdef _MS3_SUPPORT
    if (strncasecmp (mtype, "ms3", 3) == 0) {
