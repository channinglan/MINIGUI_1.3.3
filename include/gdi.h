...
/*
 * $Id: gdi.h,v 1.151 2003/11/23 06:03:39 weiym Exp $
 *
...
#define DC_ATTR_PEN_TYPE        2
#define DC_ATTR_PEN_COLOR       3
#define DC_ATTR_BRUSH_TYPE      4
#define DC_ATTR_BRUSH_COLOR     5
#define DC_ATTR_TEXT_COLOR      6
#define DC_ATTR_TAB_STOP        7
 ...
 */
#define GetPenType(hdc)     (int)       GetDCAttr (hdc, DC_ATTR_PEN_TYPE)
 ...
 */
#define GetBrushType(hdc)   (int)       GetDCAttr (hdc, DC_ATTR_BRUSH_TYPE)
 ...
 *
 * \param hdc The device context.
 * \param type The new pen type. Only \a PT_SOLID is supported so far.
 ...
 */
#define SetPenType(hdc, type)       (int)       SetDCAttr (hdc, DC_ATTR_PEN_TYPE, (DWORD) type)
 ...
 *
 * \param hdc The device context.
 * \param type The new brush type. Only \a BT_SOLID is supported so far.
 ...
 */
#define SetBrushType(hdc, type)     (int)       SetDCAttr (hdc, DC_ATTR_BRUSH_TYPE, (DWORD) type)
...
int GUIAPI GetPenType (HDC hdc);
int GUIAPI GetBrushType (HDC hdc);
gal_pixel GUIAPI SetPenColor (HDC hdc, gal_pixel color);
gal_pixel GUIAPI SetBrushColor (HDC hdc, gal_pixel color);
int GUIAPI SetPenType (HDC hdc, int pentype);
int GUIAPI SetBrushType (HDC hdc, int brushtype);
...
/**
 * \fn void GUIAPI ArcGenerator (void* context, int sx, int sy, int r, fixed ang1, fixed ang2, CB_ARC cb)
 ...
 * of the point.
 *
 * MiniGUI implements \a Arc function by using this generator.
 *
 * \param context The context, will be passed to the callback \a cb.
 * \param sx sx,sy: The center of the arc.
 * \param sy sx,sy: The center of the arc.
 * \param r The radius of the arc.
 * \param ang1 The radian of the start angle in fixed point.
 * \param ang2 The radian of the end angle in fixed point.
 * \param cb The arc generator callback.
 *
 * \note Only defined for _USE_NEWGAL
 *
 * \sa Arc, CB_ARC, fixed_math_fns
 */
 ...
 *
 * This function draws an ellipse with the current pen color on the DC \a hdc. 
...
/**
 * \fn void GUIAPI Arc (HDC hdc, int sx, int sy, int r, fixed ang1, fixed ang2)
 * \brief Draws an arc.
 *
 * This function draws an arc with the current pen color on the DC \a hdc. 
 ...
 * \param sy sx,sy: The center of the arc.
 * \param r The radius of the arc.
 * \param ang1 The radians of start angle in fixed point.
 * \param ang2 The radians of end angle in fixed point.
 *
 * \note Only defined for _USE_NEWGAL
 *
 * \sa ArcGenerator, fixed_math_fns, dc_attrs
 */
...
/**
 * \fn void GUIAPI FillArc (HDC hdc, int sx, int sy, int r, int ang1, int ang2)
 * \brief Fills an arc.
 *
 ...
 * \sa ArcGenerator, fixed_math_fns, dc_attrs
 */
void GUIAPI FillArc (HDC hdc, int sx, int sy, int r, int ang1, int ang2);
...
/**
 * \fn void GUIAPI GetGlyphBitmap (LOGFONT* log_font, const char* mchar, int mchar_len, GLYPHBITMAP* glyph_bitmap)
...
#define DC_ATTR_CHAR_EXTRA      8
#define DC_ATTR_ALINE_EXTRA     9
#define DC_ATTR_BLINE_EXTRA     10
...
/**
 * \fn int GUIAPI DrawTextEx (HDC hdc, const char* pText, int nCount, RECT* pRect, int nIndent, UINT nFormat)
